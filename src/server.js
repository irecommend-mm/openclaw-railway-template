...
function ebookProjectsRootDir() {
  return path.join(WORKSPACE_DIR, "data", "ebook-projects");
}

function safeProjectSlug(raw) {
...
function ebookProjectPaths(slug) {
  const root = ebookProjectsRootDir();
  const jsonPath = path.join(root, `${slug}.json`);
  const tocPath = path.join(root, `${slug}-toc.md`);
  const stylePath = path.join(root, `${slug}-style.md`);
  return { root, jsonPath, tocPath, stylePath };
}

function readEbookProject(slug) {
  const { jsonPath, tocPath, stylePath } = ebookProjectPaths(slug);
  let state = null;
  try {
  ...
  return { state, toc, style };
}

function writeEbookProject(slug, { state, toc, style }) {
  const { root, jsonPath, tocPath, stylePath } = ebookProjectPaths(slug);
  fs.mkdirSync(root, { recursive: true });
  if (state) fs.writeFileSync(jsonPath, JSON.stringify(state, null, 2) + "\n", "utf8");
  if (typeof toc === "string") fs.writeFileSync(tocPath, toc, "utf8");
  if (typeof style === "string") fs.writeFileSync(stylePath, style, "utf8");
}
...
app.get("/studio-app.js", requireSetupAuth, (_req, res) => {
  try {
    const p = path.join(process.cwd(), "src", "public", "studio-app.js");
    let js = fs.readFileSync(p, "utf8");
    js = js.replace("__OPENCLAW_GATEWAY_TOKEN__", OPENCLAW_GATEWAY_TOKEN);
...
app.get("/studio", requireSetupAuth, (_req, res) => {
  res.sendFile(path.join(process.cwd(), "src", "public", "studio.html"));
});
...
/** Ebook Studio (cowork mode) — project state + docs export, protected by SETUP_PASSWORD. */
app.get("/studio/api/projects", requireSetupAuth, async (_req, res) => {
  try {
    const root = ebookProjectsRootDir();
    fs.mkdirSync(root, { recursive: true });
...
app.get("/studio/api/project", requireSetupAuth, async (req, res) => {
  try {
    const slug = safeProjectSlug(req.query?.slug);
    if (!slug) return res.status(400).json({ ok: false, error: "slug_required" });
    const { state, toc, style } = readEbookProject(slug);
    return res.json({ ok: true, slug, state, toc, style });
  } catch (err) {
    return res.status(500).json({ ok: false, error: String(err) });
  }
});

app.post("/studio/api/project/save", requireSetupAuth, async (req, res) => {
  try {
    const slug = safeProjectSlug(req.body?.slug);
    if (!slug) return res.status(400).json({ ok: false, error: "slug_required" });
    const state = req.body?.state && typeof req.body.state === "object" ? req.body.state : null;
    const toc = req.body?.toc != null ? String(req.body.toc) : "";
    const style = req.body?.style != null ? String(req.body.style) : "";
    ...
    writeEbookProject(slug, { state: mergedState, toc, style });
    return res.json({ ok: true, slug, state: mergedState });
  } catch (err) {
    return res.status(500).json({ ok: false, error: String(err) });
  }
});

function buildChapterPrompt({ project, chapterNo, chapterTitle }) {
  const title = project?.bookTitle || "";
  const subtitle = project?.subtitle || "";
  const audience = project?.audience || "";
  const language = project?.language || "my-MM";
  const lessonTemplate = project?.lessonTemplate || "";
  const tone = project?.tone || "";
  const targetWords = project?.chapters?.find?.((c) => c?.no === chapterNo)?.targetWords || "";
  return [
    "You are a long-form ebook writing assistant.",
    "Write in Myanmar language (Burmese).",
    `Language: ${language}`,
    "",
    "Book metadata:",
    `- Title: ${title}`,
    ...
    "",
    "Rules:",
    "- Use the exact chapter heading format: `အခန်း (N): <ခေါင်းစဉ်>`",
    "- Include: ရည်မှန်းချက်များ, အဓိကသင်ခန်းစာများ, အသုံးချနမူနာများ, လက်တွေ့လေ့ကျင့်ရန်, အကျဉ်းချုပ်",
    "- Be concrete and instructional; avoid filler.",
    "- Do NOT change the Table of Contents.",
    "",
    targetWords ? `Target length: ~${targetWords} words.` : null,
    "",
    "Lesson template (follow closely for each main lesson block):",
    lessonTemplate || "(no template provided)",
    "",
    `Now write Chapter ${chapterNo}:`,
    `Chapter title: ${chapterTitle}`,
    "",
    "Return ONLY the chapter content (no preface).",
  ]
    .filter(Boolean)
    .join("\n");
}

/** Generate a prompt for next chapter (UI sends this to gateway WS). */
app.post("/studio/api/next-chapter", requireSetupAuth, async (req, res) => {
  try {
    const slug = safeProjectSlug(req.body?.slug);
    if (!slug) return res.status(400).json({ ok: false, error: "slug_required" });
    const { state } = readEbookProject(slug);
    if (!state) return res.status(404).json({ ok: false, error: "project_not_found" });
    const chapters = Array.isArray(state.chapters) ? state.chapters : [];
    const requestedNo = Number.parseInt(String(req.body?.chapterNo ?? ""), 10);
    const requested =
      Number.isFinite(requestedNo) && requestedNo > 0
        ? chapters.find((c) => c?.no === requestedNo)
        : null;
    const next =
      requested ||
      chapters.find((c) => String(c?.status || "pending") !== "done") ||
      chapters[0];
    if (!next) return res.status(400).json({ ok: false, error: "no_chapters_configured" });
    const prompt = buildChapterPrompt({
      project: state,
      chapterNo: next.no,
      chapterTitle: next.title || "",
    });
    return res.json({ ok: true, slug, chapter: next, prompt });
  } catch (err) {
    return res.status(500).json({ ok: false, error: String(err) });
  }
});

function buildFrontMatterPrompt(project, tocText, styleText) {
  const title = project?.bookTitle || "";
  const subtitle = project?.subtitle || "";
  const audience = project?.audience || "";
  const tone = project?.tone || "";
  const language = project?.language || "my-MM";
  return [
    "You are writing the FRONT MATTER of a long-form ebook in Myanmar language (Burmese).",
    `Language: ${language}`,
    "",
    "Book metadata:",
    `- Title: ${title}`,
    ...
    tone ? `- Tone: ${tone}` : null,
    "",
    "Use this exact section order and headings (Myanmar + optional English in parentheses):",
    "1) မိတ်ဆက် (Introduction)",
    "2) ဘယ်သူတွေအတွက်လဲ (For Whom)",
    "3) သင်ဘာတွေသင်ယူရမလဲ (What You Will Learn)",
    "4) Table of Contents (TOC) — reproduce the TOC exactly (no changes)",
    "5) အသုံးပြုနည်း / စာဖတ်နည်း (How to use this book) (short)",
    "6) အဆုံးသတ်တွင်ရရှိမည့်အကျိုးကျေးဇူးများ (short)",
    "",
    "Style guide (follow):",
    styleText || "(no style guide provided)",
    "",
    "TOC (do not modify; output it under the TOC section exactly):",
    tocText || "(no TOC provided)",
    "",
    "Return ONLY the front matter content (no commentary).",
  ]
    .filter(Boolean)
    .join("\n");
}

app.post("/studio/api/front-matter", requireSetupAuth, async (req, res) => {
  try {
    const slug = safeProjectSlug(req.body?.slug);
    if (!slug) return res.status(400).json({ ok: false, error: "slug_required" });
    const { state, toc, style } = readEbookProject(slug);
    if (!state) return res.status(404).json({ ok: false, error: "project_not_found" });
    const prompt = buildFrontMatterPrompt(state, toc, style);
    return res.json({ ok: true, slug, prompt });
  } catch (err) {
    return res.status(500).json({ ok: false, error: String(err) });
  }
});

/** Replace entire document body with front matter + reset chapter statuses to pending. */
app.post("/studio/api/export/replace-body", requireSetupAuth, async (req, res) => {
  try {
    const slug = safeProjectSlug(req.body?.slug);
    if (!slug) return res.status(400).json({ ok: false, error: "slug_required" });
    const text = req.body?.text != null ? String(req.body.text) : "";
    ...
    if (!docId) return res.status(400).json({ ok: false, error: "docId_missing_in_project_state" });
    ...
    const requests = [...deletes, { insertText: { location: { index: 1 }, text: replacement } }];
    const batchRes = await fetch(
      `https://docs.googleapis.com/v1/documents/${encodeURIComponent(docId)}:batchUpdate`,
      {
        method: "POST",
    ...
    state.lastChapterWritten = null;
    writeEbookProject(slug, { state, toc, style });
    ...
    return res.json({ ok: true, batchUpdate: batchJson, verify: verifyJson, state });
  } catch (err) {
    log.error("studio-export-replace-body", String(err));
    return res.status(500).json({ ok: false, error: String(err) });
  }
});

/** Append approved chapter to Google Doc (server-side OAuth) and mark chapter done. */
app.post("/studio/api/approve-append", requireSetupAuth, async (req, res) => {
  try {
    const slug = safeProjectSlug(req.body?.slug);
    if (!slug) return res.status(400).json({ ok: false, error: "slug_required" });
    const chapterNo = Number.parseInt(String(req.body?.chapterNo ?? ""), 10);
    if (!Number.isFinite(chapterNo)) {
      return res.status(400).json({ ok: false, error: "chapterNo_required" });
    }
    const chapterText = req.body?.chapterText != null ? String(req.body.chapterText) : "";
    const { state, toc, style } = readEbookProject(slug);
    if (!state) return res.status(404).json({ ok: false, error: "project_not_found" });
    const docId = String(state.docId || "").trim();
    if (!docId) return res.status(400).json({ ok: false, error: "docId_missing_in_project_state" });
    ...
    const index = docsAppendInsertIndex(docJson);
    const text = chapterText.endsWith("\n") ? chapterText : `${chapterText}\n`;
    const batchRes = await fetch(
      `https://docs.googleapis.com/v1/documents/${encodeURIComponent(docId)}:batchUpdate`,
      {
        method: "POST",
    ...
    state.lastChapterWritten = chapterNo;
    writeEbookProject(slug, { state, toc, style });

    return res.json({ ok: true, insertIndex: index, batchUpdate: batchJson, state });
  } catch (err) {
    log.error("studio-approve-append", String(err));
    return res.status(500).json({ ok: false, error: String(err) });
  }
});
